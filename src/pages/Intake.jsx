...
const JOBS = [
  { title: 'Junior Frontend Developer', match: 87, tags: ['React', 'CSS', 'JavaScript'] },
  { title: 'UI/UX Designer',            match: 74, tags: ['Figma', 'Prototyping'] },
  { title: 'Product Designer',          match: 61, tags: ['Design Systems', 'Research'] },
];
...
export default function Intake() {
  ...
  const [form, setForm]       = useState({ status: '', goal: '', interest: '', hours: '20', budget: 'free' });
  const fileRef = useRef();

  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  const handleFile = () => {
    setScan('scanning'); setScanPct(0);
    let p = 0;
    const iv = setInterval(() => {
      p += Math.random() * 14 + 7;
      if (p >= 100) { p = 100; clearInterval(iv); setTimeout(() => setScan('done'), 300); }
  ...
  const handleGenerate = async () => {
    setBusy(true);
    setUserData(form);
    let blocks = isAIEnabled() ? await generateRoadmap(form) : null;
    if (!blocks) blocks = DEFAULT_ROADMAP;
          ...
          {step === 1 ? (
            <div className="ref-card" style={{ padding: '32px' }}>
              <div style={{ marginBottom: 24 }}>
                ...
                <p style={{ fontSize: 13, color: '#8c96a5', lineHeight: 1.5 }}>Our AI will parse your existing skills and find matching trajectories.</p>
              </div>
              ...
              {scan === 'idle' && (
                <>
                  ...
                  >
                    ...
                    <p style={{ fontSize: 12, color: '#94a3b8' }}>PDF, DOCX or TXT · up to 5 MB</p>
                    <button
              ...
              {scan === 'scanning' && (
                <div style={{ background: '#f8fafc', padding: '36px 24px', borderRadius: '20px', textAlign: 'center', border: '1px solid #e2e8f0' }}>
                  ...
                  <p style={{ fontSize: 14, fontWeight: 700, color: '#1e2430', marginBottom: 4 }}>Analysing your profile</p>
                  <p style={{ fontSize: 12, color: '#8c96a5' }}>Extracting skills, experience, and roadmap vector…</p>
              ...
              {scan === 'done' && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: '#ecfdf5', borderRadius: '12px', marginBottom: 20 }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2.5">
                      <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/>
                      <polyline points="22 4 12 14.01 9 11.01"/>
                    </svg>
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 700, color: '#065f46' }}>Profile analysed successfully</p>
                      <p style={{ fontSize: 12, color: '#047857' }}>Found 14 skills · 3 matched career trajectories</p>
                    </div>
                  </div>

                  <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px', color: '#8c96a5', marginBottom: 10 }}>Top Trajectories</p>
                  {JOBS.map(j => (
                    <div key={j.title} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', border: '1px solid #eef2f8', borderRadius: '14px', marginBottom: 8, background: '#ffffff' }}>
                      <div>
                      ...
                      <span style={{ fontSize: 18, fontWeight: 800, color: j.match >= 80 ? '#10b981' : '#f59e0b' }}>
                        {j.match}%
                      </span>
                    </div>
                  ))}
                  ...
                  <button
                    onClick={() => setStep(2)}
                    style={{ width: '100%', padding: '12px 0', background: '#1e2430', color: '#ffffff', borderRadius: '999px', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 20 }}
                  >
            ...
            <div className="ref-card" style={{ padding: '32px' }}>
              <div style={{ marginBottom: 24 }}>
                ...
                <p style={{ fontSize: 13, color: '#8c96a5', lineHeight: 1.5 }}>Tell us where you stand and what you're aiming for.</p>
              </div>

              <div style={{ marginBottom: 18 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#4b5563', marginBottom: 6 }}>Current Status</label>
                <select
                  value={form.status}
                ...
                >
                  ...
                  <option value="dropout">Left formal education / Exploring</option>
                  <option value="gap_year">On a gap year / Upskilling</option>
                  <option value="career_switch">Switching careers into software</option>
              ...
              <div style={{ marginBottom: 18 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#4b5563', marginBottom: 6 }}>Field of Interest</label>
                <select
                  value={form.interest}
                ...
                >
                  ...
                  <option value="webdev">Fullstack & Frontend Development</option>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#4b5563', marginBottom: 6 }}>Hours / Week</label>
                  <input
                    type="number"
                    value={form.hours}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#4b5563', marginBottom: 6 }}>Resource Budget</label>
                  <select
                    value={form.budget}
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                >
                  {busy ? 'Generating your roadmap…' : 'Generate 90-Day Roadmap →'}
                </button>
              </div>
            </div>
          )}
